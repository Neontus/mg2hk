...
aia = cut_aia
global aia_dims
aia_dims = aia.shape[0], aia.shape[1]
aia = aia.flatten()

def align(flataia, scalex = 1, scaley = 1, theta = 0, translatex = 0, translatey = 0):
    aia = np.reshape(flataia, (aia_dims))
...
aligned = np.reshape(align(aia, best_values[0], best_values[1], best_values[2], best_values[3], best_values[4]), (aia_dims))

print("Similarity: ", alignlib.mse(aligned, iris))

fig, ax = plt.subplots(1, 3, figsize=[10, 10])
ax[0].imshow(np.reshape(aia, (aia_dims)), cmap='afmhot', origin='lower')
